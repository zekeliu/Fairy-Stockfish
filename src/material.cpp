  ...
  int imbalance(const Position& pos, const int pieceCount[][PIECE_TYPE_NB]) {
    ...
    for (int pt1 = NO_PIECE_TYPE; pt1 <= QUEEN; ++pt1)
    {
        ...
        for (int pt2 = NO_PIECE_TYPE; pt2 <= pt1; ++pt2)
            v +=  QuadraticOurs[pt1][pt2] * (pieceCount[Us][pt2] + pos.count_in_hand(Us, PieceType(pt2))) * (pos.must_capture() && pt1 == KNIGHT && pt2 == PAWN ? 2 : 1)
                + QuadraticTheirs[pt1][pt2] * (pieceCount[Them][pt2] + pos.count_in_hand(Them, PieceType(pt2)));
